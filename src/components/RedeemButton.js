...
export default function RedeemButton({ balanceWINES }) {
  ...
  return (
    <BuyButtonFrame>
      <ButtonFrame
        disabled={balanceWINES > 0 ? false : true}
        text={'Sell'}
        type={'secondary'}
        onClick={() => {
      ...
      <ButtonFrame
        disabled={
          account === null ||
          !balanceWINES ||
          balanceWINES.lt(ethers.utils.bigNumberify(10).pow(ethers.utils.bigNumberify(18)))
        }
        text={'Redeem'}
        type={'secondary'}
        onClick={() => {
